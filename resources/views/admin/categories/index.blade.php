...
@section('content')
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                        ...
                        <tr class="group hover:bg-slate-50/80 transition-all">
                            ...
                            <td class="px-8 py-5 text-right">
                                <div class="flex justify-end gap-1">
                                    <button onclick="openEditModal('{{ $category->id }}', '{{ $category->name }}', '{{ $category->division_pj }}')" 
                                        class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-xl transition-all" title="Edit">
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-slate-400 tracking-tight">Belum ada data kategori.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="px-8 py-5 border-t border-slate-50 bg-slate-50/30">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">Showing {{ $categories->count() }} Categories</p>
        </div>
    </div>
...
<script>
    ...
    function closeEditModal() { document.getElementById('modalEditCategory').classList.add('hidden'); }
